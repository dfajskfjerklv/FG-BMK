...
sys.path.append('./')
sys.path.append('/data/PycharmProjects/yht/LMFG/unilm/beit3')
sys.path.append('/data/PycharmProjects/yht/LMFG/unilm')
...
from dinov2.eval.utils import ModelWithIntermediateLayers, evaluate_retrieval
...
def get_args_parser(
    ...
    parser.set_defaults(
        ...
        eval_period_iterations=1e9,
        learning_rates=[1e-3, 2e-3, 5e-3, 1e-2, 2e-2, 5e-2],
        val_metric_type=MetricType.MULTILABEL_RECALL,
...
class LinearPostprocessor(nn.Module):
    ...
    def forward(self, samples, targets, gallery_feature, gallery_targets):
        M, N, D, C = targets.size(0), gallery_feature.size(0), gallery_feature.size(1), gallery_targets.size(1)
        preds = self.linear_classifier(samples)
        sim_all = -F.pairwise_distance(preds.unsqueeze(1).expand(M, N, D), 
                                       gallery_feature.unsqueeze(0).expand(M, N, D))
        
        logits = -torch.ones(M, C, device=preds.device)*1e4
        values, indices = sim_all.topk(10, dim=1)
        for i in range(9, -1, -1):
            logits = torch.where(gallery_targets[indices[:, i]] > 0.999, values[:, i].unsqueeze(1).expand(M, C), logits)
            #cls = gallery_targets[indices[:, i]] * values[:, i].unsqueeze(1)
            #logits.append(cls)
        preds = logits.softmax(-1)
        '''logits = []
        for i in range(gallery_targets.size(1)):
            preds_cls = sim_all[:, gallery_targets[:, i] > 0.999]
            if preds_cls.size(1) == 0:
                logits.append(-torch.ones(M, device=sim_all.device)*1e4)
            else:
                logits.append(-preds_cls.min(dim=1).values)
        preds = torch.stack(logits, dim=1).softmax(-1)'''
        return {
            "preds": preds,
            "target": targets,
        }
...
def evaluate_linear_classifiers(
    ...
    _, results_dict_temp = evaluate_retrieval(
        feature_model,
        query_data_loader,
        gallery_data_loader,
        postprocessors,
    ...
    for i, (classifier_string, metric) in enumerate(results_dict_temp.items()):
        ...
        if (
            best_classifier_on_val is None and metric["recall@1"].item() > max_accuracy
        ) or classifier_string == best_classifier_on_val:
            max_accuracy = metric["recall@1"].item()
            best_classifier = classifier_string
            max_map = metric["map"].item()

    results_dict["best_classifier"] = {"name": best_classifier, "accuracy": max_accuracy, "MAP": max_map}
...
def eval_linear(
    *,
    ...
    for data, pos, neg in metric_logger.log_every(
        train_data_loader,
        10,
        header,
        max_iter,
        ...
        data = torch.cat([data, pos, neg], dim=0)
        ...
        losses = {f"loss_{k}": F.triplet_margin_loss(*v.chunk(3)) for k, v in outputs.items()}
        ...
        if eval_period > 0 and (iteration + 1) % eval_period == 0 and iteration != max_iter - 1:
            _ = evaluate_linear_classifiers(
                ...
                gallery_data_loader=gallery_data_loader,
...
def make_eval_data_loader(test_dataset_str, batch_size, num_workers, metric_type):
    ...
    query_dataset = TripletDataset(meta[test_dataset_str_process], 'datasets/' + test_dataset_str, False, transform, True, True)
    ...
    gallery_dataset = TripletDataset(meta[test_dataset_str_process], 'datasets/' + test_dataset_str, True, transform, True, True)
    gallery_data_loader = make_data_loader(
    ...
    return query_data_loader, gallery_data_loader
...
def test_on_datasets(
    ...
    for test_dataset_str, class_mapping, metric_type in zip(test_dataset_strs, test_class_mappings, test_metric_types):
        ...
        query_data_loader, gallery_data_loader = make_eval_data_loader(test_dataset_str, batch_size, num_workers, metric_type)
...
def run_eval_linear(
    ...
    train_dataset = TripletDataset(meta[train_dataset_str], 'datasets/' + train_dataset_str, True, train_transform, True, False)
    ...
    query_data_loader, gallery_data_loader= make_eval_data_loader(val_dataset_str, batch_size, num_workers, val_metric_type)
...
def main(args):
    if args.arch == 'dinov2':
        ...
        from dinov2.utils.config import setup
        config = setup(args)
        from unilm.beit3.run_beit3_debug import beit3
        model = beit3()
        autocast_dtype = torch.float16
...
if __name__ == "__main__":
    ...
    def read_csv_merge_before_last_comma(file_path, expected_fields=2):
        data = []
        with open(file_path, 'r') as file:
            for line in file:
                fields = line.strip().split(',')
                if len(fields) > expected_fields:
                    fields = [','.join(fields[:-1]), fields[-1]]  # 合并最后一个逗号前的内容
                data.append(fields)
        return pd.DataFrame(data, columns=[f'Field{i+1}' for i in range(expected_fields)])
    file_path = f'./datasets/{args.train_dataset_str}/train.csv'
    df = read_csv_merge_before_last_comma(file_path)
    row_count = len(df)
    ...
    args.val_dataset_str = args.train_dataset_str+'/cub'
        
    args.output_dir = f'save/retrieval/{args.arch}/{args.train_dataset_str}'
